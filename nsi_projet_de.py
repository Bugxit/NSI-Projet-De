...
class Dices():
	...
	def first_point(self, dice_corner_position_x, dice_corner_position_y, rotation, y):
			...
			for i in range(y):
				tu.seth(rotation+i*90)
				tu.forward(120)
			tu.seth(rotation+y*90+45)
	...
	def draw_dice(self, dice_corner_position_x,dice_corner_position_y,size, rotation, number_rolled,y):
		...
		if number_rolled >= 2:
			...
			tu.seth(rotation+90*y)
			tu.forward(80)
			self.draw_circle(tu.xcor(), tu.ycor(), 15)
			self.first_point(dice_corner_position_x, dice_corner_position_y, rotation, y)
			tu.seth(rotation+90+90*y)
			tu.forward(80)
			self.draw_circle(tu.xcor(), tu.ycor(), 15)
		if number_rolled >= 4:
			self.first_point(dice_corner_position_x, dice_corner_position_y, rotation, y)
...
def generate_dice():
	...
	for i in range(1080):
		tu.clear()
		d1.draw_dice(-840, 540-i, 120, 90, number_rolled, 0)
		ti.sleep(0.001)
		tu.Screen().update()
	for i,y,teta in tool.product(range(4),range(4),range(91)):
		tu.clear()
		d1.draw_dice(-840+(4*i+y)*120, -540, 120, 90-teta, number_rolled, y)
		tu.Screen().update() 
		#ti.sleep(0.005)
for i in range(10):
